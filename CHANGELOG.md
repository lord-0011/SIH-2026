...
- what changed (code):
  ...
  - `tests/test_risk_fixture.py`: Dedicated non-skipping CI fixture suite (6 tests) verifying probability monotonic mapping, synthetic risk band classification, data sufficiency threshold logic, synthetic Platt calibration behavior, and a decoupled synthetic invariant test (`LOW < MED < HIGH < CRIT`) vs. snapshot regression test.
  - `tests/test_risk.py`: Real data integration suite (5 tests, cleanly skipped when DVC assets absent) asserting total row count (18,860), 0 nulls, calibration ECE reduction, and a hardened self-enforcing monotonicity invariant (`test_per_band_realized_rate_strict_monotonicity`) that recomputes realized slip rates directly from raw columns in `risk_scores.parquet` against the dynamically calculated base rate, decoupled from `test_per_band_snapshot_regression` which pins the exact baseline values as a tripwire.
- what was verified (real output ref):
